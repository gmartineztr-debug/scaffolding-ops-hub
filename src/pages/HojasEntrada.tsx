...
export default function HojasEntrada() {
  const [hojas, setHojas] = useState<HE[]>([]);
  const [search, setSearch] = useState("");

  useEffect(() => {
    supabase.from("hojas_entrada").select("*").order("created_at", { ascending: false })
      .then(({ data }) => setHojas(data || []));
  }, []);

  const filtered = hojas.filter((h) =>
    h.folio_he.toLowerCase().includes(search.toLowerCase()) ||
  ...
  return (
    <div className="space-y-4 animate-fade-in">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-lg font-bold text-foreground">Hojas de Entrada</h1>
          <p className="text-sm text-muted-foreground">{hojas.length} registros</p>
        </div>
        <Button size="sm">
          <Plus className="h-4 w-4 mr-1" /> Nueva HE
        </Button>
      </div>
      ...
      <div className="bg-card rounded-lg border">
        <Table>
          <TableHeader>
            <TableRow>
              <TableHead className="font-mono">Folio HE</TableHead>
            ...
            {filtered.length === 0 ? (
              <TableRow>
                <TableCell colSpan={8} className="text-center text-muted-foreground py-8">
                  No hay hojas de entrada
                </TableCell>
              </TableRow>
            ) : (
              filtered.map((h) => (
                <TableRow key={h.id} className="hover:bg-muted/50">
                  <TableCell className="font-mono text-sm text-primary">{h.folio_he}</TableCell>
                  <TableCell className="font-mono text-sm">{h.folio_c || "—"}</TableCell>
                  <TableCell className="text-sm">{h.razon_social || "—"}</TableCell>
                  <TableCell className="text-sm font-mono">
                    {h.fecha_recepcion ? new Date(h.fecha_recepcion).toLocaleDateString("es-MX", { day: "2-digit", month: "short", year: "numeric" }) : "—"}
                  </TableCell>
